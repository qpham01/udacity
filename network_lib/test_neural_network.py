...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_network_lesson09(self):
        ...
        """
        network = NeuralNetwork('lenet5 for MNIST')
        ...
        network.define_network(features)

        learning_rate = 0.001
        network.optimize_softmax_cross_entropy(learning_rate, 'gradient_descent', labels)
        ...
        batch_size = 128
        saver = network.train(training_epochs, next_batch, data_size, batch_size, features, \
            labels, 'mnist_training')

        network.evaluate(saver, mnist.test.images, mnist.test.labels, features, labels)
