...
class FlipPredictor(object):
    def __init__(self,coins):
        ...
        n=len(coins)
        self.probs=[1/n]*n
        # print "coins: ", self.coins, "  n: ", n, "  probs: ", self.probs
    ...
    def pheads(self):
        ...
        n = len(self.coins)
        for i in xrange(n):
            pH += self.coins[i] * self.probs[i]
        return pH
        
    def update(self,result):
        #Write a function the updates
        #the probabilities of flipping each coin
        #coins[n] is probability of H given coin n, or p(H|Cn)
        #probs[n] is probabiliby of drawing coin n, or P(Cn)
        #p(Cn|H) = p(H|Cn) * p(Cn) / p(H)
        #p(H) = pheads() = p(H|Cn) * p(Cn) + p(H|Cnotn) * p(Cnotn)
        #p(Cn|T) = p(T|Cn) * P(Cn) / p(T)
        #p(T) = 1 - p(H)        
        n = len(self.coins)
        p = [];
        for i in xrange(n):
            if (result == 'H'):
                pi = self.coins[i] * self.probs[i] / self.pheads()
                p.append(pi)
            else:
                pi = (1 - self.coins[i]) * self.probs[i] / (1 - self.pheads())
                p.append(pi)
        for i in xrange(n):
            self.probs[i] = p[i]
...
print test(testcases[0][0][0], testcases[0][0][1])
