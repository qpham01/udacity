...
def dl_run_start(network, data, hyper_dict, user=DEFAULT_USER, environment=DEFAULT_ENV):
    """ Log the start of a deep learning network execution """
    print("Run started with %s on dataset %s using hyper-params %s by user %s on %s." % \
        (network, data, hyper_dict, user, environment))
    return 0
